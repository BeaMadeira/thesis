...
let tSize = 5; //3 5 7 10
let mSize = 5; //3 5 7 10
let snakeSize = 3; //0 2 3 5 8
let espacamento = 0.85;// 10 4 3 2 1 0
let palete;
...
async function cropCanvas(inputCanvas, outputCanvas, faceBox, landmarks, marginWidth = 0.05, marginHeight = 0.05) {
...
click_button.addEventListener('click', async function () {
	...
	//get parameters from user if they r not empty
 	let aux_var1 = document.getElementById("myNumber").value;
	//console.log(aux_var1);
	if (aux_var1 ) {
		//convert to int
		tSize=parseInt(aux_var1);
	}

	let aux_var3 = document.getElementById("myNumber2").value;
	//console.log(aux_var3);
	if (aux_var3 ) {
		mSize=parseInt(aux_var3);
	}
	//IF MSIZE != TSIZE -> COMENT DIFFERENT TILE SIZES
	if (mSize != tSize) {
		difSizes = false;
	}

	let auxz = document.getElementById("myNumber3").value;
	//console.log(auxz);
	if (auxz ) {
		snakeSize=parseInt(auxz);
	}

	let aux_var = document.getElementById("myNumber4").value;
	//console.log(aux_var);
	if (aux_var ) {
		espacamento=parseInt(aux_var);
	}
	...
	await cropCanvas(canvas, canvas2, resizedDetections[0].detection.box, resizedDetections[0].landmarks._positions, 0.3, 0.5)
	...
	//get image data
	const image_data = contextlandmarks.getImageData(0, 0, canvas2.width, canvas2.height);
	const image_data_fg = ctxfg.getImageData(0, 0, canvas2.width, canvas2.height);
...
/*  	  if (image_data_fg.data[(i * w + j) * 4 + 2] == 255) {
	...
	/*     let dot1x = Math.cos(angle) * (dot1[0] - j) - Math.sin(angle) * (dot1[1] - i) + i;
		let dot1y = Math.sin(angle) * (dot1[0] - j) + Math.cos(angle) * (dot1[1] - i) + j;
    
		let dot2x = Math.cos(angle) * (dot2[0] - j) - Math.sin(angle) * (dot2[1] - i) + i;
		let dot2y = Math.sin(angle) * (dot2[0] - j) + Math.cos(angle) * (dot2[1] - i) + j;
    
		let dot3x = Math.cos(angle) * (dot3[0] - j) - Math.sin(angle) * (dot3[1] - i) + i;
		let dot3y = Math.sin(angle) * (dot3[0] - j) + Math.cos(angle) * (dot3[1] - i) + j;
    
		let dot4x = Math.cos(angle) * (dot4[0] - j) - Math.sin(angle) * (dot4[1] - i) + i;
		let dot4y = Math.sin(angle) * (dot4[0] - j) + Math.cos(angle) * (dot4[1] - i) + j; */
...
function fillForm(){
  ...
  filename = "70_1_4"+tSize+"_"+mSize+"_"+espacamento+"_"+snakeSize+"_"+t10+".jpg";
...
/*     const file = document.getElementByIWd( "ficheiro" ).files[ 0 ];
